  ...
  try {
    ...
    if (user.Role === "Manager") {
      ...
      if (items !== null) {
        ...
        res
          .status(404)
          .json({ message: "There are no any recordes please add items" });
      }
    } else {
      res.status(401).json("Only Manager has access to do this operation");
    }
...
export const getItemByCategory = async (req, res) => {
  try {
    ...
    if (user.Role === "Manager") {
      ...
      const findItems = await Item.find({ Category: Category }).populate(
        "Category"
      );
      if (findItems !== null) {
        let Items = [];
...
export const updateItem = async (req, res) => {
  try {
    ...
    if (user.Role === "Manager") {
      const { SerialNo } = req.params;
      const findItem = await Item.findOneAndUpdate(
        { SerialNo: SerialNo },
        {
          ...req.body,
        },
