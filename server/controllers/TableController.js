...
export const AddTable = async (req, res) => {
  try {
    ...
    if (user.Role === "Manager" || user.Role === "Admin") {
      ...
      console.log(TableNo);
      const existingTable = await Table.findOne({ TableNo: TableNo }).populate(
        "TableNo"
      );
      if (existingTable) {
        res.status(400).json({
  ...
  try {
    ...
    if (
      ...
      res.status(401).json({
        status: "Error",
        message:
          "Only Manager, Staff-Member & Admin have access to do this operation",
      });
    }
  } catch (error) {
    res.status(501).json({
  ...
  try {
    ...
    if (user.Role === "Customer") {
      ...
      if (tables !== null) {
        ...
        res
          .status(404)
          .json({ message: "There are no any recordes please add tables" });
      }
    } else {
      res
        .status(401)
