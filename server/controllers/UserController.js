  ...
  try {
    ...
    if (user.Role === "Admin" || user.Role === "Manager") {
      ...
      if (existingServiceProvider !== null) {
        ...
        if (Role === "Admin") {
          const user = await ServiceProviders.findOne({ Role: Role }).populate(
            "Role"
          );
          console.log(user);
          if (user !== null) {
...
export const getUsersByRole = async (req, res) => {
  try {
    ...
    if (user.Role === "Admin" || user.Role === "Manager") {
      ...
      const Users = await ServiceProviders.find({ Role: Role }).populate(
        "Role"
      );
      if (Users !== null) {
        let users = [];
  ...
  try {
    ...
    if (user.Role === "Admin" || user.Role === "Manager") {
      ...
      if (findServiceProvider) {
        const removeUser = await ServiceProviders.findByIdAndRemove(
...
export const getAvailableDeliverers = async (req, res) => {
  try {
    ...
    if (user.Role === "Staff-Member") {
      const Deliverers = await ServiceProviders.find({
        Role: "Deliverer",
      }).populate("Role");
      if (Deliverers !== null) {
        let deliverers = [];
...
export const UpdateProfile = async (req, res) => {
  try {
    ...
    const { Email } = req.params;
    if (user.Email == Email) {
      const logedCustomer = await Customer.findOne({ Email: Email }).populate(
        "Email"
      );
      const logedServiceProvider = await ServiceProviders.findOne({
        Email: Email,
      }).populate("Email");
