...
const TableBooking = (props) => {
  ...
  const handleClickCombination = (index) => {
    const isItemAlreadySelected = items.some(
      (item) => item.TableNo === Item[index].TableNo
    );
    if (isItemAlreadySelected) {
      // Remove the item from the array
      setItems((prevItems) =>
        prevItems.filter((item) => item.TableNo !== Item[index].TableNo)
      );
  ...
  return (
    <l.Container>
                  ...
                  >
                    <Grid item xs={6} md={4}>
                      {clickedIndex[index] && click ? (
                        <l.ClickedItem>
                          <l.H2>
                            Table on {table.TableNo} <br /> seats ={" "}
                            {table.NoOfPersons}
                            <br /> Price : {table.price}
                          </l.H2>
                        </l.ClickedItem>
                      ) : (
                        <l.Item>
      ...
      <l.Div6>
        <l.RegularButton onClick={handleConfirmPayment}>
          CONFORM
        </l.RegularButton>
      </l.Div6>
